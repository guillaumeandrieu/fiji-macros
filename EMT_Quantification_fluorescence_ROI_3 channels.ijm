...
macro "EMT Fluoresence Quantification 3 channels Action Tool-C037T0b11FT7b093Tcb09c" {
...
fileName=getDirectory("Where are the files ?") ;
listeImages=getFileList(fileName) ;


for (n=0; n< listeImages.length; n++){
while (nImages>0) {
       	   selectImage(nImages);
      	    close(); 
}
	open(fileName+ listeImages[n]);
...
choiceC3 = newArray(c1Signal, c2Signal, c3Signal, c4Signal, c5Signal, c6Signal, c7Signal, c8Signal);
  Dialog.create("C3"); 
        Dialog.addChoice("Signal name", choiceC3); 
        Dialog.show(); 
       C3 = Dialog.getChoice(); 	
...
channelChoice = newArray(C1, C2, C3);
...
saveAs("Tiff", fileName + listeImages[n]+"_"+chosenSignal+"-ROI");
close();
...
saveAs("Measurements", fileName + listeImages[n]+"_"+chosenSignal+"-Results.xls");
...
selectWindow(C1);
run("8-bit");
saveAs("Tiff", fileName + listeImages[n]+"_"+C1);
close();

selectWindow(C2);
run("8-bit");
saveAs("Tiff", fileName + listeImages[n]+"_"+C2);
close();

selectWindow(C3);
run("8-bit");
saveAs("Tiff", fileName + listeImages[n]+"_"+C3);
close();
...
// Sélection du canal à analyser
channelChoice1 = newArray(C1, C2, C3);
...
channelChoice2 = newArray(C1, C2, C3);
...
run("Analyze Particles...", "size=30-Infinity circularity=00-1.00 show=[Overlay Outlines] add");
selectWindow("Threshold");
saveAs("Tiff",fileName + listeImages[n]+"_"+chosenSignal1+"-ROI");
close();
selectWindow(chosenSignal1);
roiManager("Measure");
...
saveAs("Measurements", fileName + listeImages[n]+"_"+chosenSignal1+"-Results.xls");
...
run("Analyze Particles...", "size=30-Infinity circularity=00-1.00 show=[Overlay Outlines] add");
selectWindow("Threshold");
saveAs("Tiff",fileName + listeImages[n]+"_"+chosenSignal2+"-ROI");
close();
...
saveAs("Measurements", fileName + listeImages[n]+"_"+chosenSignal2+"-Results.xls");
